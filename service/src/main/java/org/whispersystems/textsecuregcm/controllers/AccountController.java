...
import org.whispersystems.textsecuregcm.configuration.ControlPanelConfiguration;
...
public class AccountController {
  ...
  private final Optional<DirectoryQueue>           directoryQueue;
  ...
  private final ControlPanelConfiguration          controlPanel;
  ...
  public AccountController(PendingAccountsManager pendingAccounts,
                           ...
                           ControlPanelConfiguration controlPanel)
  {
    ...
    this.directoryQueue                    = Optional.ofNullable(directoryQueue);
                                ...
                                @HeaderParam("Authorization") String authorizationHeader)
  {
    if (controlPanel.isEnabled()) {
      try {
        ...
        Optional<String> accessKey    = Optional.ofNullable(header.getIdentifier().getUuid().toString());
        Optional<String> accessSecret = Optional.ofNullable(header.getPassword());

        if (!accessKey.orElse("").equals(controlPanel.getAuthKey()) || !accessSecret.orElse("").equals(controlPanel.getAuthSecret())) {
        ...
        logger.info("Bad Control Panel Authorization Header", e);
      }
    }
  ...
  public void setGcmRegistrationId(@Auth DisabledPermittedAccount disabledPermittedAccount, @Valid GcmRegistrationId registrationId) {
    ...
    if (directoryQueue.isPresent() && !wasAccountEnabled && account.isEnabled()) {
      directoryQueue.get().addRegisteredUser(account.getUuid(), account.getNumber());
    }
  }
  ...
  public void deleteGcmRegistrationId(@Auth DisabledPermittedAccount disabledPermittedAccount) {
    ...
    if (directoryQueue.isPresent() && !account.isEnabled()) {
      directoryQueue.get().deleteRegisteredUser(account.getUuid(), account.getNumber());
    }
  }
  ...
  @DELETE
  @Path("/cp/{number}")
  public Response deleteRegistrationId(@PathParam("number")          String number,
                                       @HeaderParam("Authorization") String authorizationHeader) {
    if (controlPanel.isEnabled()) {
      try {
        AuthorizationHeader header    = AuthorizationHeader.fromFullHeader(authorizationHeader);
        ...
        Account account = accounts.get(number).get();
        Device device = account.getMasterDevice().get();
        String gcmId = device.getGcmId();
        String apnId = device.getApnId();
        ...
        if (gcmId != null) {
          gcmSender.sendMessage(new GcmMessage(gcmId, number, 0, GcmMessage.Type.COMMAND, Optional.of("LOGOUT")));
        } else if (apnId != null) {
          apnSender.sendMessage(new ApnMessage(apnId, number, 0, true, Optional.of("LOGOUT"), true));
        }
        ...
        logger.info("Account/Device Not Found");
  ...
  private Account createAccount(String number, String password, String userAgent, AccountAttributes accountAttributes) {
    ...
    if (directoryQueue.isPresent()) {
      if (account.isEnabled()) {
        directoryQueue.get().addRegisteredUser(account.getUuid(), number);
      } else {
        directoryQueue.get().deleteRegisteredUser(account.getUuid(), number);
      }
    }
