...
public class ProfileController {
  ...
  public ProfileController(RateLimiters rateLimiters,
                           ...
                           CdnConfiguration profilesConfiguration) throws Exception {
    ...
    this.minioClient      = new MinioClient(profilesConfiguration.getEndpoint(),
                                            profilesConfiguration.getAccessKey(),
  ...
  @Produces(MediaType.APPLICATION_JSON)
  @Path("/avatar")
  public ProfileAvatarUploadDescriptor getAvatarUploadDescriptor(@Auth Account account) throws Exception {
    String               previousAvatar = account.getAvatar();
    String               objectName     = generateAvatarObjectName();
    ...
    return new ProfileAvatarUploadDescriptor(objectName, minioClient.presignedPutObject(bucket, objectName, 60 * 60 * 1));
  }

  @Timed
  @GET
  @Produces(MediaType.APPLICATION_JSON)
  @Path("/avatar/profiles/{avatarId}")
  public ProfileAvatarUri getAvatarUri(@Auth                  Account account,
                                       @PathParam("avatarId") String  avatarId)
          throws Exception
  {
    String               key = "profiles/" + avatarId;

    return new ProfileAvatarUri(minioClient.presignedGetObject(bucket, key, 60 * 60 * 1));
  }
